...
export const refreshToken = (response: AxiosResponse) => {
  ...
  const loginParam = {
    LoginID: userStore.UserID,
    Password: userStore.Password,
    Type: 0, // 账号密码登录
    Source: appStore.platform, // 账号密码登录
    DeviceID: appStore.deviceUUID
  };
  return userStore.loginValidate(loginParam, true)
    .then(() => {
      // 重新请求
      // const { RetCode, ReturnData } = res;
      // if (RetCode === ResultEnum.SUCCESS) {
        // userStore.setToken(ReturnData.LoginToken);
...
export const requestSuccess = (request: AxiosRequestConfig) => {
  ...
  request.url = userStore.baseUrl + request.url
  addPending(request);
  return request;
};
// 请求失败拦截
