<template>
    <div class="my-modal-box">
        ...
        <div class="my-modal-wrapper">
            ...
            <div
                class="modal-content"
                v-if="myPackages.type === 'prompt' || myPackages.type === 'confirm' || myPackages.type === 'textarea'"
            >
                <div v-if="myPackages.type === 'prompt'">
                    <ion-input
                        v-model="inputValue"
                        ref="myInput"
                        class="fits-custom-alert-input"
                        :type="myPackages.subType ? myPackages.subType : 'text'"
                        :placeholder="myPackages.placeholder"
                    ></ion-input>
                </div>

                <div v-if="myPackages.type === 'textarea'">
                    <ion-textarea
                        v-model="inputValue"
                        ref="myTextarea"
                        class="fits-custom-alert-textarea"
                        :placeholder="myPackages.placeholder"
                    ></ion-textarea>
                </div>

                <div
                    v-if="myPackages.type === 'confirm'"
                    class="fits-custom-alert-confirm"
                >{{ myPackages.content }}</div>
            </div>
            ...
            <ion-list no-lines v-if="myPackages.type && myPackages.type === 'radio'">
                <button
                    ion-item
                    mode="md"
                    v-for="(item, i) in myPackages.radioArray"
                    :key="i"
                    @click="changeValue($event, item, i)"
                >
                    <ion-label
                        :style="{ color: item.isSelected ? '#29a1f7' : '#333' }"
                    >{{ item.name }}</ion-label>
                    <ion-icon
                        :icon="checkmark"
                        item-end
                        ...
                        v-if="!myPackages.isShowBtn && item.isSelected"
                    ></ion-icon>
...
import { modalController, IonBackdrop, IonInput, IonTextarea } from "@ionic/vue";
...
const inputValue = ref(myPackages.inputVal); // 输入框的值
...
const ok = () => {
    if (myPackages.type && myPackages.type === "radio") {
        ...
        modalController.dismiss(resultArray);
    } else if (myPackages.type && (myPackages.type === "prompt" || myPackages.type === "textarea")) {
