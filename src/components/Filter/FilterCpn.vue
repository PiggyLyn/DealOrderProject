            ...
            <div v-if="currentFilter.item.btnID==='other'" class="other">
                ...
                >
                    ...
                    <div class="btnContainer">
                        <ion-button
                        @click="otherClicked(ele)"
                        class="otherItem"
                        v-for="ele in item.btns"
                        :key="ele.btnID"
                        fill="outline"
                        >
                            <ion-icon :icon="checkmarkOutline" v-show="otherBtnArr.arr.includes(ele.btnID)"></ion-icon>
                            {{ele.btnName}}
                        </ion-button>
                    </div>
                </div>
...
const showFilter = (item:any) => {
    if (item.btnID === currentNode && isShow.value) {
 ...
 * @desc 其他筛选:点击里面的按钮
 */
const otherClicked = (item:any) => {
    // item.isSelected = !item.isSelected
    if (!otherBtnArr.arr.includes(item.btnID)) {
        otherBtnArr.arr.push(item.btnID)
    } else {
        const index = otherBtnArr.arr.indexOf(item.btnID)
        console.log(index)
        otherBtnArr.arr.splice(index, 1)
    }
}
